...
| Layer | Công nghệ |
|-------|-----------|
| AI | DeepSeek API (OpenAI-compatible, thay thế Anthropic Claude) |
...
```
DEEPSEEK_API_KEY         ← DeepSeek API (thay thế Anthropic Claude)
...
| G | DB Schema | ✅ **28 local / 25 cloud** | `supabase/migrations/` — migration #28+#29+#30 chưa push cloud ⚠️ |
...
| TRIPGENIE | AI Chat Core | ✅ **v2.0.0** | `/api/ai/chat` — **deepseek-chat** (DeepSeek-V3) + RAG ✅ |
| DEEPSEEK-ENGINE | DeepSeek API Client | ✅ **v1.0.0** | `src/lib/ai/deepseek.ts` — `callDeepSeek()` + `streamDeepSeek()` |
| DB-OPTIMIZATIONS | 16 DB indexes | ✅ **v1.0.0** | `migration #30` — pg_trgm; composite; partial indexes ⚠️ cloud pending |
...
| DX-CODE-QUALITY | ESLint zero-warning sweep | ✅ **v1.0.0** | 22 files `type NextRequest`; 5× `catch(_e)`; 2× `console.warn`; unused imports; `pdf-index.types.ts` |
...
| `/api/ai/chat` | POST | ✅ **v2.0.0** | Node.js + RAG + SSE — **deepseek-chat** (DeepSeek-V3) |
...
DEEPSEEK_API_KEY: ✅ .env.local — ⚠️ CẦN ADD lên Vercel
ANTHROPIC_API_KEY: ✅ .env.local (dùng cho classify/itinerary/content/landing-page)
...
Env vars CẦN THÊM Vercel: DEEPSEEK_API_KEY, ZALO_OA_SECRET, ZALO_OA_ACCESS_TOKEN, FB_VERIFY_TOKEN, FB_APP_SECRET
...
pnpm build (Handover #74): ✅ CLEAN — 0 TypeScript errors
pnpm lint  (Handover #72): ✅ CLEAN — 0 ESLint warnings or errors

AI models (phiên #74):
  src/lib/ai/claude.ts              → deepseek-chat (DeepSeek-V3) — MỚI ✅
  src/lib/ai/deepseek.ts            → callDeepSeek() + streamDeepSeek() — MỚI ✅
...
```
# ƯU TIÊN #1 — APPLY MIGRATION #28 + #29 + #30 LÊN SUPABASE CLOUD:
  ...
  3. supabase/migrations/20260624000030_db_optimizations.sql

# ƯU TIÊN #2 — THÊM DEEPSEEK_API_KEY LÊN VERCEL:
  Vercel Dashboard → Settings → Environment Variables → Add:
  DEEPSEEK_API_KEY = sk-a73403d3f32... (lấy từ .env.local)
  → Redeploy để chat AI hoạt động trên production

# ƯU TIÊN #3 — ĐĂNG KÝ TELEGRAM WEBHOOK:
  curl -X POST "https://api.telegram.org/bot{TOKEN}/setWebhook" \
       -d "url=https://namngantravel.com/api/webhooks/telegram"

# ƯU TIÊN #4 — SEED BẢNG zalo_accounts (sau khi apply #29):
...
# ƯU TIÊN #5 — THÊM VERCEL ENV VARS CÒN LẠI:
...
1. **Apply migration #28 + #29 + #30 cloud** — #28 → LandingPages; #29 → Omnichannel; #30 → 16 performance indexes (pg_trgm)
2. **Add `DEEPSEEK_API_KEY` lên Vercel** — chat AI production 503 nếu thiếu; lấy từ `.env.local`
3. **Seed `zalo_accounts` + đăng ký Telegram webhook** — Flow B (staff reply → Zalo) không hoạt động nếu thiếu

### Change Log

| Ngày | Giai đoạn | Thay đổi |
|------|-----------|---------|
| 2026-06-24 | Handover #74 — DB Optimizations migration #30 ✅ | 16 indexes: pg_trgm; leads.zalo_id/fb_lead_id; composite tours/leads/tickets |
| 2026-06-24 | Handover #73 — DeepSeek Integration v1.0.0 ✅ | deepseek.ts; claude.ts → DeepSeek; StreamMessage → ai.types.ts |
| 2026-06-24 | Handover #72 — DX Code Quality sweep ✅ | ESLint 0 warnings: 22 files type imports; 5× catch(_e); 2× console.warn |
| 2026-06-24 | Handover #71 — DX Toolchain ✅ | .clinerules + .continue + .vscode + .prettierrc + ESLint @typescript-eslint |
| 2026-06-24 | Handover #70 — Omnichannel Gateway v1.0.0 ✅ | migration #29; 4 bảng; Telegram↔Zalo bidirectional; 5 CRM routes |
| 2026-06-23 | Handover #69 — Landing Page Factory ✅ | /promo/[slug] ISR; AI extract; LandingPageTab CRM; địa chỉ+email mới |
| 2026-06-22 | Handover #68 — Itinerary Editor + Image fix ✅ | ItineraryEditor admin; day.images render; tất cả model → opus-4-8 |
| 2026-06-16 | Handover #67 — Tour Detail v5.0.0 + Upload fix ✅ | 2-col scrollspy; TourBookingWidget+ItineraryNav mới; upload auth fix |
| 2026-06-16 | Handover #66 — Listing UI đồng nhất ✅ | HScrollRow ← → arrows; 4 trang filter+sort chuẩn hoá |
| 2026-06-16 | Handover #65 — Agent.py Image Scraper ✅ | scripts/agent.py v1.1.0; migration #27 images format |
| 2026-06-15 | Handover #64 — Gallery Layout Fix ✅ | TourGallery 2/3→60/40; seed script |
| 2026-06-15 | Handover #63 — Health Check + Bug Fix ✅ | 15 pages + 9 APIs green; fix tours PATCH adminClient |
| 2026-06-13 | Handover #62 — Tour Gallery ✅ | migration #26; TourGalleryManager drag+upload |
| 2026-06-12 | Handover #61 — TripAgent Chat Perf ✅ | ChatBubble.memo+SmartScroll; appendDelta O(1) |
| 2026-06-12 | Handover #60 — Homepage FPS Fix ✅ | shadow-overlay×3; backdrop-blur removed; content-visibility |
| 2026-06-12 | Handover #59 — Tour Detail 500 fix + Import ✅ | force-dynamic; 36 tours imported |
| 2026-06-12 | Handover #58 — Tour Detail v3.0.0 ✅ | TourLeadBox+TabNav+Timeline; 83 tours Sheets |
| 2026-06-11 | Handover #55 — Tour Ecosystem Linking DONE | sitemap /tour/[slug]; TourListingCard slug prop |
| 2026-06-11 | Handover #52 — Crawler DONE ✅ | crawl-seastar-tours.ts ✅; migration IF NOT EXISTS fix |
| 2026-06-11 | Handover #50 — Sheets Sync LIVE ✅ | Sheets sync live; migrations #16-21 cloud ✅ |
